  ...
  return (
    <FormControl>
      ...
      >
        {data.map((x) => (
          <FormControlLabel
            sx={{ "& .MuiFormControlLabel-label": { marginLeft: "-7px" } }}
            key={x.categoryId}
            value={x.categoryId}
            control={<Radio />}
...
const initialValue = "<p>type something here...</p>";
export default function EditPost() {
  ...
  //
  const { handleSubmitNewPost, handleFetchPostType, postType } =
    usePost(apiUrl);
  ...
  const { handleFetchPostById, singlePostData } = usePost(apiUrl);
  ...
  useEffect(() => {
    if (singlePostData) {
      // setSelectedBlog(singlePostData.blog_id)
      setPostContent(singlePostData.post_content);
      setPostDescription(singlePostData.post_description);
      setPostProfilePictureURL(singlePostData.post_profile_picture_url);
    }
  }, [singlePostData]);
    ...
    <>
      ...
      <Container sx={{ mt: 0 }}>
        ...
        <ThemeProvider theme={defaultTheme}>
          <NewBlogsRadioButtonsGroup
            options={myBlogData}
            selectedBlog={selectedBlog}
            setSelectedBlog={setSelectedBlog}
          />
          <br />
          <CategoryRadioButtonsGroup
            data={blogCategory}
            setSelectedCategory={setSelectedCategory}
          />
          <br />
          <PostTypeRadioButtonsGroup
            data={postType}
